...
namespace MS_ActWatch.Helpers
{
    ...
    public static partial class LogManager
    {
        ...
        public static void LoadConfig()
        {
            ...
            LM_CFG.Clear();
            string sConfig = $"{Path.Join(ActWatch._dllPath!, "log_config.json")}";
            string sData;
            ...
            if (bNotFound)
            {
                ...
                if (iSend == 1)
                {
                    CfgTest.LWritter = new LoggerConfiguration()
                        .WriteTo.File($"{ActWatch._sharpPath!}/logs/ActWatch/{CfgTest.Send}-.txt", rollingInterval: RollingInterval.Day, outputTemplate: "{Timestamp:yyyy-MM-dd HH:mm:ss.fff(zzz)} [{Level:w3}] {Message:l}{NewLine}{Exception}")
                        .CreateLogger();
                }
            }
        }
        ...
        static async Task SendToDistord(string sWebHook, string sMessage, bool bMapName = true)
        {
            try
            {
                ...
                if (bMapName && ActWatch._modSharp!.GetMapName() is { } mapname) sMapname = mapname;
        ...
        public static void CvarAction(string sCvarName, string sCvarValue)
        {
            foreach (LogCfg cfg in LM_CFG.ToList())
            {
                if (cfg.CvarInfo)
                {
                    string sMsg = ServerLocalizer.Format(CultureInfo.GetCultureInfo(cfg.Lang), "ActWatch.Cvar.Notify", sCvarName, sCvarValue);
                    if (string.Equals(cfg.Type.ToLower(), "file") && cfg.LWritter != null) cfg.LWritter.Information(sMsg);
                    else if (string.Equals(cfg.Type.ToLower(), "discord"))
                    {
                        Task.Run(async () =>
