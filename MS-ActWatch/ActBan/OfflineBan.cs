...
namespace MS_ActWatch.ActBan
{
    ...
    public static class OfflineFunc
    {
        static OfflineBan CreateOrFind(IGameClient UserID)
        {
            ...
            {
                if (string.Equals(OfflineTest.SteamID, AW.ConvertSteamID64ToSteamID(UserID.SteamId.ToString())))
                {
        ...
        public static OfflineBan? FindTarget(IGameClient admin, string sTarget, bool bChat)
        {
            ...
            else if (sTarget[0] == '#')
            {
                ...
                {
                    if (AW.g_Scheme != null) UI.ReplyToCommand(admin, "ActWatch.Reply.Must_be_an_integer", bChat, 2, AW.g_Scheme.Color_warning);
                    return null;
                }
                foreach (OfflineBan OfflineTest in AW.g_OfflinePlayer.ToList())
                {
            ...
            else
            {
                ...
                if (iCount > 1)
                {
                    if (AW.g_Scheme != null) UI.ReplyToCommand(admin, "ActWatch.Reply.More_than_one_client_matched", bChat, 2, AW.g_Scheme.Color_warning);
                    return null;
                }
            }
